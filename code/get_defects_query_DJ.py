...
tms = ["Mo", "W"]
dichalc = ["S", "Se", "Te"]
criteria = {"elements":{"$in":["Mo","W"],"$in":["S","Se","Te"]},"nelements":2}
...
with MPRester() as mp:
	...
	tmds=mp.query(criteria,properties,chunk_size=100,max_tries_per_chunk=5)
	for i in tmds:
		print(i["material_id"],i["pretty_formula"],i["energy"])

#Creating directories with mp-ids

		str_file=os.path.join(os.path.join(basepath,'%s')%(i["material_id"]))
...
#Creating bulk_str files with the structure_formula
		...
		bulk_structure = mp.get_structure_by_material_id(i["material_id"]) 
...
#Renaming the folders according to their structural formula
		os.rename(os.path.join(os.path.join(basepath,'%s')%(i["material_id"])),os.path.join(basepath,'%s'+'-'+'%s')%(i["material_id"],str_formula))
	print(structures)
